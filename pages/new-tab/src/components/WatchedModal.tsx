...
const WatchedModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const [cards, setCards] = useState<any[]>([]); // API에서 가져온 시청 기록 데이터
  const [currentIndex, setCurrentIndex] = useState(0); // 현재 캐러셀의 인덱스
  const [isLoading, setIsLoading] = useState(true); // 로딩 상태

  // 시청 기록 API 호출
  const fetchWatchedRecords = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/v1/shorts/records', {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          'X-CSRFTOKEN': 'A6wVjoITASNZVqEiNiidbx1e7b6Utd0E9CEEjGn1Sop0qJEfsIbPTKkG82nJUWM7',
        },
      });

      if (!response.ok) {
        throw new Error('Failed to fetch watched records');
      }

      const data = await response.json();
      setCards(data.records); // 시청 기록 데이터 설정
      ...
      >
        ...
        <div className="w-full h-full flex overflow-hidden relative items-center justify-center">
          <div
            ...
            style={{
              transform: `translateX(calc(50% - ${currentIndex * 296 + 130}px))`, // 정확한 중앙 계산
            }}>
