...
const WishedModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const [cards, setCards] = useState<any[]>([]); // API에서 가져온 위시리스트 데이터
  ...
  const fetchWishedList = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/v1/shorts/wishes', {
        method: 'GET',
        headers: {
      ...
      setCards(data.wishlist.reverse()); // 데이터 배열을 역순으로 정렬
